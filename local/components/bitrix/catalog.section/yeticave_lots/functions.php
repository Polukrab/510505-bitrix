...
if (!function_exists('getTimeStringBeforExp')) {
	function getTimeStringBeforExp($time_1, $time_2) {
		$dt = $time_2 - $time_1;
		$time['sec'] = $dt % 60;
		$time['min'] = floor(($dt % 3600) / 60);
		$time['hours'] = floor(($dt % 86400) / 3600);
		$time['days'] = floor($dt / 86400);

		foreach ($time as $key => $value) {
			if ($value < 10 && $key != 'days') {
				$time[$key] = "0{$value}";
			}
		}
		...
		$result ="";
		if ($time['days']) {
			$result = "{$time['days']}д {$time['hours']}:{$time['min']}";
		} else {
			$result = "{$time['hours']}:{$time['min']}:{$time['sec']}";
		}

		return Array('STRING' => $result, 'SECONDS' => $dt);
	}
}
?>